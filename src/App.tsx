...
interface ScriptStep {
  ...
  next?: number; // Default next step index
  field?: keyof UserProfile; // Which profile field to update
  options?: ScriptOption[]; // Choices for the user
  setter?: (val: any) => void; // Custom function to update state
  dummy?: boolean; // If true, data isn't saved (flavor text)
}
...
// --- LOGIC ENGINE ---
// (Simplified version of previous engine for brevity, assuming same scoring rules)
const calculateScore = (p: UserProfile) => {
    ...
    // Simple interpolation for age
    let agePts = 0;
    if(p.age >= 20 && p.age <= 29) agePts = withSpouse ? 100 : 110;
    else if(ageMapSingle[p.age]) agePts = withSpouse ? ageMapSpouse[p.age] : ageMapSingle[p.age];
    else if(p.age > 29 && p.age < 40) {
        // Approx drop
        const diff = p.age - 29;
        agePts = withSpouse ? (100 - (diff * 5)) : (110 - (diff * 6)); // Rough est
    }
    score += Math.max(0, agePts);
    ...
    score += withSpouse ? (eduScoreSpouse[p.education as keyof typeof eduScoreSpouse] || 0) : (eduScore[p.education as keyof typeof eduScore] || 0);

    // C. LANGUAGE (Assume English is First for calculation simplicity in this mock, or swap based on input)
    ...
    score += langPoints(p.english.speak) + langPoints(p.english.listen) + langPoints(p.english.read) + langPoints(p.english.write);
    
    // Second language (French)
    const lang2Points = (clb: number) => (clb>=9 ? 6 : (clb>=7 ? 3 : (clb>=5 ? 1 : 0)));
    score += lang2Points(p.french.speak) * 4; // *4 for all abilities simplified
    ...
    score += withSpouse ? workMapSpouse[yrs] : workMap[yrs];

    // E. TRANSFERABILITY (Simplified logic)
    // CLB 9+ and 2+ degrees
    const clb9 = p.english.speak >= 9; // Assuming all 9 for simplicity in this visual novel
    const twoDeg = p.education === 'TwoOrMore' || p.education === 'Masters' || p.education === 'PhD';
    if(clb9 && twoDeg) score += 50;
    else if(clb9 && p.education !== 'None') score += 25;

    // Foreign Work + CLB 9
    if(clb9 && p.workForeign >= 3) score += 50;
    else if(clb9 && p.workForeign >= 1) score += 25;

    // F. ADDITIONAL
    if(p.pnp) score += 600;
    if(p.french.speak >= 7 && p.english.speak >= 5) score += 50;
    if(p.siblingInCanada) score += 15;
    if(p.canadianEducation === '3YearOrMore') score += 30;
    if(p.canadianEducation === '1or2Year') score += 15;
    if(p.certificateOfQualification && clb9) score += 50;
...
export default function App() {
  const [scene, setScene] = useState<'intro' | 'interview' | 'thinking' | 'result'>('intro');
  ...
  const SCRIPT: ScriptStep[] = [
    ...
    // 23: PNP
    {
        text: "Do you have a Provincial Nomination Certificate?",
        type: 'choice',
        options: [ {label:"Yes", val:true}, {label:"No", val:false} ],
        field: 'pnp',
        next: 24
  ...
  if (scene === 'thinking') {
      setTimeout(() => {
        ...
        if (relevantDraws.some(d => score >= d.score)) passed = true;
                           ...
                           <div className="space-y-3 h-64 overflow-y-auto">
                               {relevantDraws.map((d, i) => (
                                   <div key={i} className={`flex justify-between p-3 border-l-4 ${score >= d.score ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50'}`}>
                                       <div>
                                           <div className="font-bold text-sm">{d.stream}</div>
                               ...
                               {relevantDraws.length === 0 && <p className="text-red-500 italic">You are not eligible for any specific streams.</p>}
                           </div>
                       </div>
                       
                       <div className="bg-gray-100 p-4 rounded text-sm font-mono">
                           <h3 className="font-bold border-b border-gray-300 pb-2 mb-2">OFFICER NOTES:</h3>
                           <p>Age: {profile.age} years</p>
                           <p>Marital: {profile.maritalStatus}</p>
                           <p>Education: {profile.education}</p>
                           <p>Cdn Work: {profile.workInCanada}y | Foreign: {profile.workForeign}y</p>
                           <p>Lang 1: {mapFluencyToCLB('Extremely' as any) === profile.english.speak ? 'High' : 'Low/Mid'}</p>
                       </div>
                   </div>

                   <button 
                    onClick={() => window.location.reload()}
                    className="mt-8 w-full bg-gray-900 text-white py-4 font-bold hover:bg-black transition"
                   >
                       PROCESS NEXT APPLICANT (RESTART)
                   </button>
               </div>
          </div>
      );
            ...
            <div className="max-w-4xl mx-auto">
                ...
                {canInput && (
                    <div className="animate-fade-in-up">
                        {step.type === 'statement' && (
                            <button onClick={() => handleNext()} className="text-blue-600 font-bold hover:underline text-xl">
                                ▼ Continue
                            </button>
                        )}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {step.options?.map((opt: any) => (
                                    <button 
                                        ...
                                        className="bg-gray-100 hover:bg-gray-200 border-2 border-gray-300 p-4 text-left font-bold rounded transition"
                                    >
                                        {opt.label}
                                    </button>
                                ))}
                        ...
                        {step.type === 'input' && (
                            <div className="flex gap-4">
                                <input 
                                    type="number" 
                                    id="numInput"
                                    className="border-2 border-gray-300 p-4 text-xl rounded w-full"
                                    placeholder="#"
                                    autoFocus
                                />
                                <button 
                                    onClick={() => {
                                        ...
                                        if(el.value) handleNext(parseInt(el.value));
                                    }}
                                    className="bg-red-700 text-white px-8 font-bold rounded hover:bg-red-800"
                                >
                                    Submit
                                </button>
                            </div>
